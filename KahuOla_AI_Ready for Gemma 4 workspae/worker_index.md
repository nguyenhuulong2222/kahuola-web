...
function handleHealth(env: Env, cors: CorsHeaders): Response {
  return jsonResp({
    ...
    generated_at: new Date().toISOString(),
    upstreams: {
      firms: !!env.[REDACTED_KEY],
      hms: true,
      goes: true,
      pacioos: true,
      airnow: !!env.[REDACTED_KEY],
      wfigs: true,
      nws: true,
    },
  }, 200, cors);
...
async function handleFirmsHotspots(url: URL, env: Env, cors: CorsHeaders): Promise<Response> {
  if (!env.[REDACTED_KEY]) return err(503, '[REDACTED_KEY] not configured', cors);
  ...
  const firmsUrl = `https://firms.modaps.eosdis.nasa.gov/api/area/csv/${env.[REDACTED_KEY]}/${dataset}/${west},${south},${east},${north}/${days}`;
...
const WMS_UPSTREAMS: Record<string, { url: string; ttl: number; keySecret?: keyof Env; keyParam?: string }> = {
  firms: { url: 'https://firms.modaps.eosdis.nasa.gov/mapserver/wms/South_America/', ttl: 300, keySecret: '[REDACTED_KEY]', keyParam: 'MAP_KEY' },
...
async function handleAirnowXyz(z: string, x: string, y: string, env: Env, cors: CorsHeaders): Promise<Response> {
  ...
  if (!env.[REDACTED_KEY]) return err(503, 'Service temporarily unavailable', cors);

  const cacheUrl = `https://tiles.airnowtech.org/airnow/today/${zi}/${xi}/${yi}.png`;
  const fetchUrl = `${cacheUrl}?api_key=${env.[REDACTED_KEY]}`;
